...
export function conflictReasonLabel(reason, severity) {
  if (severity === "attention" || severity === "alert") {
    return "Path mismatch: the certificate in your browser differs from public WebNotary observation.";
  }
  if (severity === "info") {
    return "Certificate differs from one WebNotary has observed; this host may use multiple legitimate leaves.";
  }
  switch (reason) {
    ...
    default:
      return "The certificate your browser sees disagrees with WebNotary public observation.";
  }
}
...
/**
 * Upsert by situation signature. Same host+leaves → bump lastSeen/count.
...
export async function recordConflict(entry) {
  ...
  const severity = entry.severity || "attention";
  const reasonLabel =
    entry.reasonLabel || conflictReasonLabel(entry.reason, severity);
...
/** Sticky OS notify only for elevated severities. */
export function shouldStickyNotify(severity) {
  return severity === "attention" || severity === "alert";
}
...
export function formatConflictsArchive(conflicts) {
  ...
  conflicts.forEach((c, i) => {
    ...
    lines.push(
      `Cause:     ${c.reasonLabel || conflictReasonLabel(c.reason, c.severity)}`,
    );
    ...
    if (known.length) {
      known.forEach((fp, j) => lines.push(`Known[${j}]:  ${fp}`));
    } else {
